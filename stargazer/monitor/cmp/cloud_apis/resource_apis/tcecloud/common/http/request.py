...
class ApiRequest(object):
    def __init__(self, host, req_timeout=60, debug=False):
        self.conn = (
            ProxyHTTPConnection(host, timeout=req_timeout)
            if protocol == "http"
            else ProxyHTTPSConnection(host, timeout=req_timeout)
        )
    ...
    def _request(self, req_inter):
        ...
        if req_inter.method == "GET":
            req_inter_url = "{}?{}".format(req_inter.uri, req_inter.data)
            ...
            raise TceCloudSDKException("ClientParamsError", "Method only support (GET, POST)")
    ...
    def send_request(self, req_inter):
        try:
            ...
            self.conn.close()
            raise TceCloudSDKException("ClientNetworkError", str(e))
...
class ResponseInternal(object):
    ...
    def __str__(self):
        headers = "\n".join("{}: {}".format(k, v) for k, v in self.header.items())
        return "Status: {}\nHeader: {}\nData: {}\n".format(self.status, headers, self.data)
