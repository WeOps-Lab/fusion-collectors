...
from aliyunsdkcore.request import AcsRequest, CommonRequest
...
from aliyunsdkcore.vendored.requests.structures import CaseInsensitiveDict, OrderedDict
...
class AcsClient:
    ...
    def __init__(
        ...
        self._signer = SignerFactory.get_signer(credential, region_id, self._implementation_of_do_action, debug)
        self._endpoint_resolver = DefaultEndpointResolver(self)

        if self._auto_retry:
            self._retry_policy = retry_policy.get_default_retry_policy(max_retry_times=self._max_retry_num)
        else:
    ...
    def user_agent_header():
        base = "{} ({} {};{})".format("AlibabaCloud", platform.system(), platform.release(), platform.machine())
        return base
    ...
    def implementation_of_do_action(self, request, signer=None):
        # keep compatible
        warnings.warn("implementation_of_do_action() method is deprecated", DeprecationWarning)
    ...
    def _get_request_read_timeout(self, request):
        ...
        path = '"{}"."{}"."{}"'.format(request.get_product().lower(), request.get_version(), request.get_action_name())
    ...
    def _handle_retry_and_timeout(self, endpoint, request, signer):
        ...
        if self._retry_policy.should_retry(retry_policy_context) & RetryCondition.SHOULD_RETRY_WITH_CLIENT_TOKEN:
        ...
        while True:
            ...
            status, headers, body, exception = self._handle_single_request(
                endpoint, request, request_read_timeout, request_connect_timeout, signer
            )
    ...
    def _handle_single_request(self, endpoint, request, read_timeout, connect_timeout, signer):
        ...
        logger.debug(
            "Request received. Product:%s Endpoint:%s Params: %s", request.get_product(), endpoint, str(params)
        )
        ...
        try:
            ...
            logger.error(
                "HttpError occurred. Host:%s SDK-Version:%s ClientException:%s",
                endpoint,
                aliyunsdkcore.__version__,
                str(exception),
            )
            ...
            if http_status == codes.BAD_REQUEST and server_error_code == "SignatureDoesNotMatch":
                if string_to_sign == server_error_message.split(":")[1]:
                    ...
                    server_error_message = (
                        "The AccessKeySecret is incorrect. " "Please check your AccessKeyId and AccessKeySecret."
                    )
            exception = ServerException(
                server_error_code, server_error_message, http_status=http_status, request_id=request_id
            )
    ...
    def do_action_with_exception(self, acs_request):
        ...
        if exception:
            raise exception
        logger.debug("Response received. Product:%s Response-body: %s", acs_request.get_product(), body)
        return body
    ...
    def do_action(self, acs_request):
        warnings.warn(
            "do_action() method is deprecated, please use do_action_with_exception() instead.", DeprecationWarning
        )
        status, headers, body, exception = self._implementation_of_do_action(acs_request)
        return body
